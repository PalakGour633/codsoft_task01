...
try:
    with tqdm(total=50,desc="Loading Train Data") as pbar:
        train_data = pd.read_csv('train_data.csv',sep=':::',header=None, names=['SerialNumber','MOVIE_NAME','GENRE','MOVIE_PLOT'],engine='python')
        pbar.update(50)
...
genre_lables = [genre.split(', ') for genre in train_data ['GENRE']]
mlb = MultiLabelBinarizer()
y_train = mlb.fit_transform(genre_lables)
...
try:
   with tqdm(total=50,desc="Loading Test Data") as pbar:
      test_data = pd.read_csv('test_data.csv',sep=':::',header=None, names=['serial','MOVIE_NAME','MOVIE_PLOT'],engine ='python')
      pbar.update(50)
except Exception as e:
...
with open("model_evaluation.txt", "a", encoding="utf-8") as output_file:
    ...
    output_file.write(f"Accuracy: {accuracy * 100:21}%\n")
    output_file.write(f"Precision: {precision: .2f}\n") 
    output_file.write(f"Recall: {recall:.2f}\n")
    output_file.write(f"F1-score: {f1:.2f}\n")
  

print("Model evaluation results and metrics have been saved to 'model evaluation.txt'.")